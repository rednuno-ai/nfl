...
export function TrainingModal({ week, options, onChoose }: { week: number; options: TrainingFocusChoice[]; onChoose: (focusId: TrainingSelection) => void }) {
  return (
    <div className="modal-backdrop">
      <div className="modal" role="dialog" aria-modal="true" aria-labelledby="training-modal-title" aria-describedby="training-modal-description">
        <div className="modal-eyebrow">Training · Week {week}</div>
        <div id="training-modal-title" className="modal-title">How do you want to spend this training week?</div>
        <div id="training-modal-description" className="modal-body">Pick a focus, or take a week for life away from the field. Every option has a real trade-off.</div>
